...
router.get(
  ...
  (req: Request, res: Response, next: express.NextFunction): void => {
    ...
    "time_slot_day".day_number,"time_slot_day".day_name  FROM "event" JOIN "event_day_slot" 
    ON "event".id = "event_day_slot".event_id JOIN "time_slot_day" ON "event_day_slot".time_slot_day 
    ...
    pool
      ...
      .then((result) => {
        ...
        for (let index = 0; index < recurring_time_slot.length; index++) {
          ...
          let query: string = `INSERT INTO "event_day_slot" (event_id, time_slot_day) VALUES ($1, $2)`;
